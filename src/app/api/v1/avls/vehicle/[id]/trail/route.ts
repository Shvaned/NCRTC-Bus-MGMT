...
export async function GET(
  ...
  try {
    ...
    const trail = await avlsService.getVehicleTrail(id, user.id)
    if (!trail) {
      return NextResponse.json(errorResponse("NOT_FOUND", "Vehicle not found"), { status: 404 })
    }
    return NextResponse.json(successResponse(trail))
  } catch (err) {
    return NextResponse.json(errorResponse("INTERNAL_ERROR", "Failed to get trail"), { status: 500 })
  }
}
