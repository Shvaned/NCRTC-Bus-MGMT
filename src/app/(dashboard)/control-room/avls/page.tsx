...
export default function ControlRoomAVLSPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden -m-4 md:-m-6 z-0">
      <div className="flex items-center gap-3 px-4 py-2 border-b border-border bg-card shrink-0 z-0">
        ...
        <div className="relative w-48">
          ...
          <Input placeholder="Search vehicle..." className="pl-7 h-8 text-xs" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 relative bg-muted z-0">
          {liveLoading ? (
            ...
            <LiveMap vehicles={vehicles} selectedVehicleId={selectedVehicleId} onVehicleClick={handleVehicleClick} />
          )}
        </div>
        ...
        <div className="w-80 border-l border-border bg-card overflow-auto shrink-0 hidden lg:block">
          ...
          <div className="divide-y divide-border">
            {vehicleList}
            {vehicles.length === 0 && <div className="p-4 text-center text-sm text-muted-foreground">No vehicles found</div>}
          </div>
        </div>
      </div>
